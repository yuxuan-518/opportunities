...
const T = {
  ...
  zh: {
    ...
    anyLocation: '任意地点',
    openOnly: '仅看开放中',
    openOnlyActive: '✅ 仅看开放中',
    clear: '✕ 清除',
    results: (n: number) => `${n} 个结果`,
    loading: '加载中...',
    noResults: '未找到相关活动',
    noResultsSub: '请尝试调整筛选条件',
    grade: (g: string) => `${g}年级`,
    grades: (gs: string[]) => `年级：${gs.join('、')}`,
    free: '免费', paid: '付费', aidAvailable: '可申请资助',
    online: '线上', inPerson: '线下', hybrid: '混合',
    deadlineDue: (d: string) => `截止 ${d}`,
    deadlineClosed: (d: string) => `已截止 ${d}`,
    visitWebsite: '访问官方网站 →',
  ...
  es: {
    ...
    subtitle: 'Oportunidades extracurriculares para estudiantes de preparatoria',
    admin: 'Admin',
...
const TYPE_LABELS: Record<Lang, Record<string, string>> = {
  en: {
    competition: 'Competition', program: 'Program', internship: 'Internship',
    scholarship: 'Scholarship', volunteer: 'Volunteer', research: 'Research',
    workshop: 'Workshop', other: 'Other'
  },
  zh: {
    competition: '竞赛', program: '项目', internship: '实习',
    scholarship: '奖学金', volunteer: '志愿者', research: '科研',
    workshop: '工作坊', other: '其他'
  },
  es: {
    competition: 'Competencia', program: 'Programa', internship: 'Pasantía',
    scholarship: 'Beca', volunteer: 'Voluntariado', research: 'Investigación',
                ...
                {langButtons.map(lb => (
                  <button key={lb.code} onClick={() => setLang(lb.code)}
                    style={{
                      ...
                      background: lang === lb.code ? 'rgba(255,255,255,0.25)' : 'transparent',
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: '16px 20px', marginBottom: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
          ...
          {hasActiveFilters && (
            <button onClick={() => { setFilterType(''); setFilterField(''); setFilterGrade(''); setFilterCost(''); setFilterLocation(''); setFilterOpenOnly(false); setSearch('') }}
              style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #ffcccc', background: '#fff5f5', color: '#e74c3c', fontSize: 13, cursor: 'pointer' }}>{t.clear}</button>
          )}
